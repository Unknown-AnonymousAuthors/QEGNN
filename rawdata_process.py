...
def get_data(dataset_name: str, recal=False) -> List[Data]:
    ...
    if dataset_name in ['MUTAG', 'PROTEINS', 'NCI1', 'NCI109', 'ENZYMES', 'IMDB-BINARY', 'IMDB-MULTI', 'PTC_MR'] \
        ...
        transform = SpectralDesign()
        dataset = EXPDataset(root="datasets/raw/EXP/", pre_transform=transform)
    elif dataset_name == "CSL":
        dataset = GNNBenchmarkDataset(root='datasets/raw', name='CSL')
        ...
        transform = SpectralDesign()
        dataset = Grapg8cDataset(root="datasets/raw/graph8c/", pre_transform=transform)
    elif dataset_name == "SR25":
        transform = SpectralDesign()
        dataset = SRDataset(root="datasets/raw/sr25/", pre_transform=transform)
