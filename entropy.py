...
# TODO: optimize codes for eig="appro_deg_ge0"
...
def cal_node_centrality(data_list: List[Data], eig: str, hop: int = 1) -> list:
    ...
    # Calculate traditional node centrality for comparing with quantum mechanics
    if eig in ['betweenness', 'current_flow_betweenness']:
...
def cal_traditional_node_centrality(data_list: List[Data], method: str):
    ...
    elif method == 'current_flow_betweenness':
    ...
    node_centrality = []
    for graph in tqdm(data_list, desc="Calculating traditional node centrality", ncols=110, leave=False):
...
def cal_traditional_edge_centrality(data_list: List[Data], method: str):
    ...
    edge_centrality = []
    for graph in tqdm(data_list, desc="Calculating traditional edge centrality", ncols=110, leave=False):
        g = to_networkx(graph, to_undirected=True)
        if nx.is_connected(g):
            ec = api_centrality(g)
            ec = list(ec.values())
        else:
            ec = [1.0 / graph.num_edges] * graph.num_edges
